...
export interface RequestEditModalProps {
  ...
  editStatusOptions: BookingStatus[];
...
export function RequestEditModal({
  ...
  const detailsDisabled =
    typeof canEditDetails === "function" && !canEditDetails(selectedRequest);
  const statusOptionsToUse = canChangeStatus ? editStatusOptions : statusOptions;
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <FormField label="Attendees">
                      <input
                        ...
                        className="w-full rounded-lg border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 outline-none transition-colors focus:border-slate-300 disabled:cursor-not-allowed disabled:opacity-60"
                      />
                    </FormField>
            ...
            <aside className="space-y-4">
              <section className="rounded-[22px] border border-slate-200 bg-white p-4">
                ...
                <div className="mt-4">
                  <FilterDropdown
                    value={draftStatus ?? selectedRequest.status}
                    onChange={(next) => setDraftStatus(next as BookingStatus)}
                    options={statusOptionsToUse.map((status) => ({
